...
function onToggleShadows(v: boolean) {
  if (!viewer.value) return;
  viewer.value.shadows = v;
}

function onToggleTerrainShadows(enabled: boolean) {
  if (!viewer.value) return;
...
<template>
  <CollapsiblePanel title="Scene">
    ...
    >
      <el-option v-for="key in Object.keys(highlighted)" :key="key" :label="key" :value="key" />
    </el-select>
    ...
    <el-select
      ...
      @change="onPickHidden"
    >
      <el-option v-for="key in Object.keys(hidden)" :key="key" :label="key" :value="key" />
    </el-select>
