...
##### IMPORTANT CHANGES
* [Google Fusion Tables](https://support.google.com/fusiontables/answer/2571232) will be unavailable after Dec 3, 2019. It is recommended to backup thematic data stored in such tables using alternative cloud services, such as Google Spreadsheets, etc.

* The Web Client is now capable of fetching data using [Google Sheets API v4](https://developers.google.com/sheets/api) and a PostgreSQL database with a RESTful API enabled ([PostgREST](http://postgrest.org/en/v6.0/)). 
The fetched data can be displayed on the infobox as thematic data when a city object is clicked.
